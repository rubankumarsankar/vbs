...
export default function EditBlogPost({ params }) {
    ...
    useEffect(() => {
        fetch(`/api/admin/blog`) // In a real app we'd build a GET /api/admin/blog/[id] or just pass data via Server Components for edits. We'll do a simple client fetch all and find for MVP since this is Phase 2.
            .then(res => res.json())
            .then(data => {
        ...
        fetch('/api/admin/blog/categories').then(res => res.json()).then(setCategories).catch(console.error)
        fetch('/api/admin/blog/authors').then(res => res.json()).then(setAuthors).catch(console.error)
    }, [id])
